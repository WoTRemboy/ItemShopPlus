...
class ImageLoader {
    ...
    static func mapLoadImage(urlString: String?, completion: @escaping (UIImage?) -> Void) -> URLSessionDataTask? {
        ...
        if let imageFromCache = getMapImageFromCache(from: urlString) {
            completion(imageFromCache)
            return nil
        }
        ...
        let task = URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            DispatchQueue.main.async {
                ...
                setMapImageCache(image: loadedImage, key: urlString)
                completion(loadedImage)
            }
        }
    ...
    private static func setMapImageCache(image: UIImage, key: String) {
        if imageCache == nil {
    ...
    private static func getMapImageFromCache(from key: String) -> UIImage? {
